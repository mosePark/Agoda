...
simulation_id <- format(Sys.time(), "좌우반전 %H시 %M분 %S초") # 날짜 기반 폴더 이름 생성
...
dat_mirrored <- dat1
dat_mirrored$X1 <- -dat_mirrored$X1


p2 <- ggplot(dat_mirrored, aes(x = X1, y = X2, color = Cluster)) +
  geom_point(size = 1.5) +
  # stat_ellipse(type = "norm", linetype = "dashed", size = 1) +  # Add ellipses
  scale_color_manual(values = cluster_colors) +
  labs(x = expression(X[1]), y = expression(X[2]), title = "Mirrored Data (Left-Right)") +
  theme_minimal(base_size = 15) +
  theme(
    panel.grid.major = element_line(color = "grey80", linetype = "solid"),
    panel.grid.minor = element_line(color = "grey90", linetype = "solid"),
