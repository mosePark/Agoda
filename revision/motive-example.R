...
setwd("C:/Users/UOS/Desktop/motive")
...
plot(raw_dat2, xlab='', ylab='', cex=4, xaxt='n', yaxt='n', ylim = c(-6,6), xlim=c(-6,6), cex.lab = label_size, font.lab = label_font, col="white")
...
plot(raw_dat1, cex=4, xlab='', ylab='', xaxt='n', yaxt='n', xlim = c(-6,6), ylim =c(-6, 6), cex.lab = label_size, font.lab = label_font, col=yarrr::transparent("darkgrey", trans.val = 0.5), bg=yarrr::transparent("darkgrey", trans.val = 0.5), pch=16)
...
plot(raw_dat3, xlab ='', ylab = '',cex=4 , xlim=c(-6, 6), ylim=c(-6, 6), xaxt='n', yaxt='n', cex.lab = label_size, font.lab = label_font, col="white")

points(raw_dat3[setdiff(ind1, overlap_indices_1_3), ], pch=21, cex=4, col=yarrr::transparent("#1b5faa", trans.val = 0.5), bg=yarrr::transparent("#1b5faa", trans.val = 0.5))
points(raw_dat3[setdiff(ind2, overlap_indices_2_3), ], pch=21, cex=4, col=yarrr::transparent("#90cef1", trans.val = 0.5), bg=yarrr::transparent("#90cef1", trans.val = 0.5))
points(raw_dat3[setdiff(ind3, overlap_indices_3_3), ], pch=21, cex=4, col=yarrr::transparent("#ffc20e", trans.val = 0.5), bg=yarrr::transparent("#ffc20e", trans.val = 0.5))

points(raw_dat3[overlap_indices_1_3, ], pch=21, cex=4, col=adjustcolor("#1b5faa", alpha.f = 0.5), bg=adjustcolor("#1b5faa", alpha.f = 0.5))
points(raw_dat3[overlap_indices_2_3, ], pch=21, cex=4, col=adjustcolor("#90cef1", alpha.f = 0.5), bg=adjustcolor("#90cef1", alpha.f = 0.5))
points(raw_dat3[overlap_indices_3_3, ], pch=21, cex=4, col=adjustcolor("#ffc20e", alpha.f = 0.5), bg=adjustcolor("#ffc20e", alpha.f = 0.5))
...
plot(raw_dat1, cex=4,xlab='', ylab='', xaxt='n', yaxt='n', xlim = c(-6,6), ylim = c(-6,6), cex.lab = label_size, font.lab = label_font, col="white")
...
plot(raw_dat1, cex=4, xlab ='', ylab = '', xlim=c(-6, 6), ylim=c(-6, 6), xaxt='n', yaxt='n', cex.lab = label_size, font.lab = label_font, col="white")

points(raw_dat1[1:n, ], pch=21, cex=4, col=yarrr::transparent("#1b5faa", trans.val = 0.5), bg=yarrr::transparent("#1b5faa", trans.val = 0.5))
points(raw_dat1[(n + 1):(2 * n), ], pch=21, cex=4, col=yarrr::transparent("#90cef1", trans.val = 0.5), bg=yarrr::transparent("#90cef1", trans.val = 0.5))
points(raw_dat1[(2 * n + 1):(3 * n), ], pch=21, cex=4, col=yarrr::transparent("#ffc20e", trans.val = 0.5), bg=yarrr::transparent("#ffc20e", trans.val = 0.5))
...
set.seed(2)
shuffsize = 40
shuff_ind = sample(c((n-shuffsize+1):n, (n-shuffsize+1):n+n, (n-shuffsize+1):n+2*n))

#new_ind1 = new_ind[1:n]
#new_ind2 = new_ind[(n + 1):(2 * n)]
#new_ind3 = new_ind[(2 * n + 1):(3 * n)]

new_ind1 = c(1:(n-shuffsize), shuff_ind[1:shuffsize])
new_ind2 = c(1:(n-shuffsize)+n, shuff_ind[1:shuffsize+shuffsize])
new_ind3 = c(1:(n-shuffsize)+2*n, shuff_ind[1:shuffsize+2*shuffsize])
# Generate raw_dat3_new with rotation applied to selected indices

raw_dat3_new = raw_dat3
raw_dat3_new[new_ind1, ] = rmvnorm(n, mean = cent1, sigma = 3*sig1) %*% rotmat
raw_dat3_new[new_ind2, ] = rmvnorm(n, mean = cent2, sigma = 3*sig2) %*% rotmat
raw_dat3_new[new_ind3, ] = rmvnorm(n, mean = cent3, sigma = 3*sig3) %*% rotmat
...
a <- raw_dat1[i, ]             # Cluster 1 대표 포인트
b <- raw_dat1[j, ]         # Cluster 2 대표 포인트
c <- raw_dat1[m, ]     # Cluster 3 대표 포인트 (변경된 위치)

# Find the corresponding transformed points in raw_dat2 and raw_dat3_new
a_2 <- raw_dat2[i, ]            # raw_dat2에서 변환된 a의 위치
b_2 <- raw_dat2[j, ]        # raw_dat2에서 변환된 b의 위치
c_2 <- raw_dat2[m, ]    # raw_dat2에서 변환된 c의 위치 (변경된 위치)

a_3 <- raw_dat3_new[i, ]  # raw_dat3_new에서 변환된 a의 위치
b_3 <- raw_dat3_new[j, ]  # raw_dat3_new에서 변환된 b의 위치
c_3 <- raw_dat3_new[m, ]  # raw_dat3_new에서 변환된 c의 위치 (변경된 위치)
...
png("alter.png", width = 2800, height = 1500)
...
# # Add index labels
# for (idx in 1:nrow(raw_dat2)) {
#   text(raw_dat2[idx, 1], raw_dat2[idx, 2], labels = idx, pos = 3, cex = 1, col = "black")
# }
...
# # Add index labels
# for (idx in 1:nrow(raw_dat1)) {
#   text(raw_dat1[idx, 1], raw_dat1[idx, 2], labels = idx, pos = 3, cex = 1, col = "black")
# }
...
# 1 by 3 (new)
par(fig = c(0.66, 1, 0.5, 1), new = TRUE)
plot(raw_dat3_new, cex=4,xlab='', ylab='', xaxt='n', yaxt='n', xlim = c(-6, 6), ylim = c(-6, 6), cex.lab = label_size, font.lab = label_font, col="white")

# # Add index labels
# for (idx in 1:nrow(raw_dat3_new)) {
#   text(raw_dat3_new[idx, 1], raw_dat3_new[idx, 2], labels = idx, pos = 3, cex = 1, col = "black")
# }
