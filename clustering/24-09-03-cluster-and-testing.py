...
# 데이터 로드
os.chdir('C:/Users/mose/agoda/data/')  # 경로 설정
...
kmeans_ori = KMeans(n_clusters=5, random_state=240902).fit(ori_embeddings_reduced)
kmeans_gen1 = KMeans(n_clusters=5, random_state=240902).fit(gen_1_embeddings_reduced)
kmeans_gen2 = KMeans(n_clusters=5, random_state=240902).fit(gen_2_embeddings_reduced)
...
# 클러스터 내에서만 레이블을 무작위로 섞기 위한 함수
def cluster_preserving_permutation(labels):
    permuted_labels = np.copy(labels)  # 원래 레이블을 복사해서 사용
    unique_labels = np.unique(labels)  # 고유한 레이블 추출
    
    for label in unique_labels:
        indices = np.where(labels == label)[0]  # 현재 레이블을 가진 인덱스 찾기
        shuffled_indices = np.random.permutation(indices)  # 인덱스를 무작위로 섞기
        permuted_labels[indices] = permuted_labels[shuffled_indices]  # 무작위 인덱스를 사용하여 레이블 재할당
    
    return permuted_labels

# 테스트를 위한 예제 레이블
labels_example = np.array([0, 0, 1, 1, 2, 2, 2, 3, 3])

# 여러 번 함수를 호출하여 결과가 달라지는지 확인
for i in range(5):
    print(cluster_preserving_permutation(labels_example))
