...
#%% 10% random sampling, seed : 240902

df = df.sample(frac=0.1, random_state=240902)
...
for index, row in tqdm(df.iterrows(), total=len(df), desc="Processing Reviews"):
    ...
    # 현재 생성된 리뷰를 파일로 저장
    df.to_csv("similar_text.csv", index=False, encoding='utf-8-sig')

# 최종 생성된 리뷰를 파일로 안전하게 저장
df.to_csv("similar_text_final.csv", index=False, encoding='utf-8-sig')
