...
prompt_template = """
어떤 유저의 국적에 대한 정보와 유저가 작성한 리뷰 텍스트를 갖고 10점 만점에 몇점을 줬을지 예측해줘. 단, 소수점 첫째자리까지:
...
# 템플릿을 사용하여 프롬프트 생성
template = PromptTemplate(template=prompt_template, input_variables=["review", "country"])
...
for index, row in tqdm(df.iterrows(), total=len(df), desc="Processing Reviews"):
    review = row['Text']
    country = row['Country']
    result = llm_chain.run({"review": review, "country": country})
    ...
    # 현재까지의 예측 결과를 파일로 저장
    df.to_csv("국가+리뷰 예측.csv", index=False, encoding='utf-8-sig')

# 최종 예측 결과를 파일로 저장 (안전하게)
df.to_csv("국가+리뷰 예측 최종.csv", index=False, encoding='utf-8-sig')
