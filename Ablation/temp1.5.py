...
llm = ChatOpenAI(api_key=api_key, model_name="gpt-3.5-turbo", temperature=1.5)
...
for index, row in tqdm(df.iterrows(), total=len(df), desc="Processing Reviews"):
    ...
    # 현재 생성된 리뷰를 파일로 저장
    df.to_csv("1.5-gen1.csv", encoding='utf-8-sig')

# 최종 생성된 리뷰를 파일로 안전하게 저장
df.to_csv("1.5-gen1-final.csv", encoding='utf-8-sig')


#%%

'''
Prompt 2 - 원본 리뷰를 기반으로 유사한 문맥의 리뷰를 생성
'''
prompt_template = """
Read the following review text and generate another review text with a similar context:
...
for index, row in tqdm(df.iterrows(), total=len(df), desc="Processing Reviews"):
    ...
    # 현재 생성된 리뷰를 파일로 저장
    df.to_csv("1.5-gen2.csv", encoding='utf-8-sig')

# 최종 생성된 리뷰를 파일로 안전하게 저장
df.to_csv("1.5-gen2-final.csv", encoding='utf-8-sig')
