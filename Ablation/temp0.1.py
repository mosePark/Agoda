...
df = pd.read_csv("0.1-gen1.csv", index_col='Unnamed: 0')
...
for index, row in tqdm(df.iterrows(), total=len(df), desc="Processing Reviews"):
    ...
    # 현재 생성된 리뷰를 파일로 저장
    df.to_csv("0.1-gen2.csv", encoding='utf-8-sig')

# 최종 생성된 리뷰를 파일로 안전하게 저장
df.to_csv("0.1-gen2_final.csv", encoding='utf-8-sig')

#%%
'''
Prompt 2 - 생성1 리뷰를 기반으로 유사한 문맥의 리뷰를 생성
'''
...
for index, row in tqdm(df.iterrows(), total=len(df), desc="Processing Reviews"):
    ...
    # 현재 생성된 리뷰를 파일로 저장
    df.to_csv("0.1-gen1-1.csv", encoding='utf-8-sig')

# 최종 생성된 리뷰를 파일로 안전하게 저장
df.to_csv("0.1-gen1-1_final.csv", encoding='utf-8-sig')
