...
if (interactive()) {
  args <- list(sz = 5000, k = 4, iter = 1000, k_near = 50)
} else {
  # Argparse 설정
...
df_7 <- read.csv("1.5-gen1-ebd.csv", header=T)
df_8 <- read.csv("1.5-gen2-ebd.csv", header=T)
df_9 <- read.csv("1.5-gen1-1-ebd.csv", header=T)
df_6 <- read.csv("df_6.csv", header=T)


sample_ind = sample(1:nrow(df_6), size=sz)

or_emb = make_matrix(df_6$ori.ebd[sample_ind])
gen1_emb = make_matrix(df_7$X1.5.gen1.ebd[sample_ind])
gen2_emb = make_matrix(df_8$X1.5.gen2.ebd[sample_ind])
gen1_1_emb = make_matrix(df_9$X1.5.gen1.1.ebd[sample_ind])
