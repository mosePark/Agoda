...
new_try_hc = function(dat1, dat2, alpha, itrN = 1000, K=3){
  ...
  dat1_cl = cutree(hclust(dist(dat1), method = "average"), k = K)
  dat2_cl = cutree(hclust(dist(dat2), method = "average"), k = K)
  
  dat1_dist = sqrt(apply((dat1 - dat1_cl[dat1_cl, ])^2, 1, sum))
  dat2_dist = sqrt(apply((dat2 - dat2_cl[dat2_cl, ])^2, 1, sum))
